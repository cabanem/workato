...
body: '<p>Build your own Google Vertex AI action with a HTTP request.
The request will ' \
...
# Authentication
{ # - auth_type
name: 'auth_type', label: 'Authentication type', group:
'Authentication',
control_type: 'select',
default: 'custom',
optional: false,
extends_schema: true,
hint: 'Select the authentication type for connecting to Google Vertex
AI.',
...
hint: 'Select the Google Cloud Platform (GCP) region used for the Vertex
model.',
...
hint: "Enter the region you want to use. See <a
...
{ # - dynamic_models
name: 'dynamic_models', label: 'Refresh model list from API (Model
Garden)', group: 'Model discovery and validation',
type: 'boolean',
control_type: 'checkbox',
optional: true,
hint: 'Fetch available Gemini/Embedding models at runtime. Falls back to
a curated static list on errors.'
},
{ # - include_preview_models
name: 'include_preview_models', label: 'Include preview/experimental
models', group: 'Model discovery and validation',
type: 'boolean',
control_type: 'checkbox',
optional: true,
sticky: true,
hint: 'Also include Experimental/Private/Public Preview models. Leave
unchecked for GA-only in production.'
},
{ # - validate_model_on_run
name: 'validate_model_on_run', label: 'Validate model before run',
group: 'Model discovery and validation',
type: 'boolean', control_type: 'checkbox', optional: true, sticky: true,
hint: 'Pre-flight check the chosen model and your project access before
sending the request. Recommended.' },
# Workato Developer API for dynamic picklists
{ # - workato_api_host
name: 'workato_api_host', label: 'Workato API host', group: 'Workato
Developer API',
hint: 'Base URL for the Workato Developer API. Defaults to
...
{ # - workato_api_token
name: 'workato_api_token', label: 'Workato API token', group: 'Workato
Developer API',
control_type: 'password',
hint: 'Personal access token with Data Tables read scope. Optional, but
needed for dynamic table/column picklists.',
optional: true,
sticky: true
}
],
...
"service-account/target='_blank'>here</a> to learn more about Google
Service " \
...
"https://#{connection['region']}-aiplatform.googleapis.com/#{connection['version']
|| 'v1'}/"
end
},
...
model = input['model']
if model.present?
"Send messages to <span
class='provider'>#{model.split('/')[-1].humanize}</span> model"
else
...
different " \
"language using Gemini models in <span class='provider'>Google Vertex
AI</span>",
help: {
...
help: {
body: 'This action helps process inputted text to find specific
information ' \
'based on defined guidelines. The processed information is then
available as datapills.'
},
...
object_definitions['parse_text_output']
          ...
          sample_output: lambda do |_connection, input|
            schema = call('safe_parse_json', input['object_schema'], 'object schema for sample output')
            call('format_parse_sample', schema).
...
"a salutation if this information isn't present in the email
description."
},
input_fields: lambda do |object_definitions|
...
help: {
body: 'This action chooses one of the categories that best fits the
input text. ' \
'The output datapill will contain the value of the best match category
or ' \
...
analyze_text: {
title: 'Analyze text',
subtitle: 'Contextual analysis of text to answer user-provided
questions',
description: "Analyze <span class='provider'>text</span> to answer
user-provided " \
...
description: "Generate text <span class='provider'>embedding</span>
using " \
"Google models in <span class='provider'>Google Vertex AI</span>",
help: {
body: 'Text embedding is a technique for representing text data as
numerical ' \
'vectors. It uses deep neural networks to learn the patterns in large
amounts ' \
'of text data and generates vector representations that capture the
meaning ' \
'and context of the text. These vectors can be used for a variety of
natural ' \
...
description: "Query a <span class='provider'>Vertex AI Vector
Search</span> index endpoint "\
"to retrieve nearest neighbors.",
retry_on_request: ['POST'],
retry_on_response: [429, 500, 502, 503, 504],
max_retries: 3,
help: {
body: "This action queries a deployed Vector Search index endpoint to
find nearest neighbors "\
"(k-NN). IMPORTANT: Use the endpoint's own host (public endpoint domain
or PSC DNS). "\
"Final URL looks like
https://{INDEX_ENDPOINT_HOST}/v1/projects/{PROJECT}/locations/{LOCATION}/"\
...
# Common pitfalls: wrong host (e.g. hitting REGION-aiplatform host) or
bad query shape
# Surface exact upstream error for troubleshooting
error("Vertex AI Vector Search error (HTTP #{code}) - #{message}:
#{body}")
end
end,
output_fields: lambda do |object_definitions|
...
description: "Get <span class='provider'>prediction in <span
class='provider'>" \
'Google Vertex AI</span>',
help: lambda do
{
...
generate_input_fields: lambda do |input|
        # The incoming `input` may be a JSON string describing the
        # schema.  Use the safe parser to avoid unhandled exceptions
        # when invalid JSON is supplied.  Default to an empty array
        # when no schema is provided.
        schema = call('safe_parse_json', input || '[]', 'dynamic field schema')
...
field
    ...
    #
    # Replace occurrences of triple backticks in a string with four hashes.  
    #
    # In several payload builders we interpolate user supplied text inside
    # code‑fences delimited by ```.  If the user includes their own
    # backticks this can prematurely terminate the fence and produce
    # malformed prompts.  Converting the backticks to an innocuous
    # sequence prevents that without altering the meaning of the text.
    #
    # We coerce the argument to a String using `to_s` so that callers
    # passing integers, booleans or other non‑string types won't raise
    # `NoMethodError` when calling `gsub`.  If the argument is nil the
    # `to_s` returns an empty string which is harmless for substitution.
    #
    ...
    #
    # Safely parse JSON.  Many of the payload builders and response
    # handlers expect certain fields (for example `responseSchema` or
    # `function.parameters`) to contain JSON encoded objects.  Invalid
    # JSON can cause `parse_json` to raise an exception which will
    # propagate to the user as an unhelpful stack trace.  By wrapping
    # the parse in a rescue block we can surface a more useful error
    # message or return nil and let the caller decide what to do.  You
    # can optionally pass a context string which will be included in the
    # error message to help locate which field failed to parse.
    #
    safe_parse_json: lambda do |raw_json, context = 'JSON payload'|
      return {} if raw_json.nil?
      begin
        # Workato provides `parse_json` which accepts either a String or a
        # Ruby object; it returns a parsed Hash/Array or nil on blank.
        parsed = parse_json(raw_json)
        parsed.nil? ? {} : parsed
      rescue StandardError => e
        error("Invalid JSON provided for #{context}: #{e.message}")
      end
    end,
    extract_json: lambda do |resp|
      # The Gemini APIs return JSON responses wrapped in markdown
      # fences.  Remove any leading ```json fences and trailing
      # backticks before attempting to parse.  If the field is absent,
      # return an empty hash.
      json_txt = resp&.dig('candidates', 0, 'content', 'parts', 0, 'text')
      return {} if json_txt.blank?
      json_clean = json_txt.to_s.gsub(/```json|```JSON|`+$/, '').strip
      call('safe_parse_json', json_clean, 'Gemini response')
    end,
...
'hate_speech' =>
ratings&.find { |r| r['category'] == 'HARM_CATEGORY_HATE_SPEECH'
}&.[]('probability'),
'harassment' =>
ratings&.find { |r| r['category'] == 'HARM_CATEGORY_HARASSMENT'
}&.[]('probability'),
'dangerous_content' =>
ratings&.find do |r|
...
when 'max_tokens'
error 'ERROR - Token generation reached the configured maximum output
tokens.'
when 'safety'
error 'ERROR - Token generation stopped because the content potentially
contains ' \
'safety violations.'
when 'recitation'
error 'ERROR - Token generation stopped because the content potentially
...
when 'blocklist'
error 'ERROR - Token generation stopped because the content contains
forbidden items.'
when 'prohibited_content'
error 'ERROR - Token generation stopped for potentially containing
prohibited content.'
when 'spii'
error 'ERROR - Token generation stopped because the content potentially
contains ' \
...
ratings = call('get_safety_ratings', resp.dig('candidates', 0,
'safetyRatings'))
...
{ 'value' => embedding }
    ...
    payload_for_send_message: lambda do |input|
      # Parse generation config JSON fields safely.  The
      # `responseSchema` can be a JSON string; if invalid the recipe
      # should raise a friendly error instead of bubbling a parsing
      # exception.  Iterate through the hash and replace only the
      # `responseSchema` key with the parsed object.
      if input&.dig('generationConfig', 'responseSchema').present?
        input['generationConfig'] = input['generationConfig'].to_a.map do |key, value|
          if key == 'responseSchema'
            { key => call('safe_parse_json', value, 'generationConfig.responseSchema') }
          else
            { key => value }
          end
        end.inject(:merge)
      ...
      # Normalize tool declarations.  Iterate using `each` to avoid
      # accidentally discarding the original array.  Parse JSON
      # parameters using the safe parser defined above.
      if input['tools'].present?
        input['tools'].each do |tool|
          next unless tool['functionDeclarations'].present?
          tool['functionDeclarations'].each do |function|
            if function['parameters'].present?
              function['parameters'] = call('safe_parse_json', function['parameters'], "function parameters for #{function['name'] || 'unnamed function'}")
            end
          end
        end
      end

      # Build contents array based on the message type.  A single
      # message yields a one element array with a user role.  For
      # multi‑message conversations iterate through the chat transcript
      # and assemble a parts array containing text, file data and
      # inline data.  Function calls and responses are parsed as JSON
      # where appropriate.
      ...
      input['contents'] = if input['message_type'] == 'single_message'
        ...
        messages['chat_transcript'].to_a.map do |m|
          {
            'role' => m['role'],
            'parts' => [
              { 'text' => m['text'] }.compact.presence,
              { 'fileData' => m['fileData'] }.compact.presence,
              { 'inlineData' => m['inlineData'] }.compact.presence,
              {
                'functionCall' => m['functionCall']&.map do |key, value|
                  if key == 'args'
                    { key => call('safe_parse_json', value, 'functionCall.args') }
                  else
                    { key => value }
                  end
                end&.inject(:merge)
              }.compact.presence,
              {
                'functionResponse' => m['functionResponse']&.map do |key, value|
                  if key == 'response'
                    { key => call('safe_parse_json', value, 'functionResponse.response') }
                  else
                    { key => value }
                  end
                end&.inject(:merge)
      ...
      # Remove fields not needed in the API payload.  We avoid
      # removing `tools` here because it may contain mutated objects
      # with parsed parameters that should still be included.
      input.except('model', 'message_type', 'messages', 'fileData',
                   'inlineData', 'functionResponse')
    end,
...
'parts' => [
{
'text' => "```#{call('replace_backticks_with_hash',
input['text'])}```\nOutput " \
...
'parts' => [
{
'text' => 'You are an assistant that helps generate summaries. All user
input ' \
...
'parts' => [
{
'text' => 'You are an assistant helping to extract various fields of
information ' \
...
'parts' => [
{
'text' => 'You are an assistant helping to analyze the provided
information. ' \
...
payload_for_analyze_image: lambda do |input|
      # Build a payload for image analysis.  Validate that both the
      # MIME type and image binary are present; raising a clear error
      # early helps users correct their configuration.  When encoding
      # the image, prefer the built‑in `encode_base64` if available on
      # the object (as provided by the Workato SDK).  Fallback to
      # Ruby's Base64 module for raw strings.  We explicitly require
      # `base64` here to avoid a NameError in environments where the
      # constant is not auto‑loaded.
      payload_for_analyze_image: lambda do |input|
        mime = input['mime_type'].to_s.strip
        image_obj = input['image']
        error('mime_type is required') if mime.blank?
        error('image is required') if image_obj.blank?

        encoded_image = nil
        if image_obj.respond_to?(:encode_base64)
          encoded_image = image_obj.encode_base64
        else
          # If encode_base64 is not defined (e.g. a raw string), use
          # Ruby's Base64 module to perform a strict encoding.  The
          # strict variant avoids line breaks in the output which can
          # break JSON encoding.
          require 'base64'
          encoded_image = Base64.strict_encode64(image_obj.to_s)
        end
        {
          'contents' => [
            {
...
# queries[].datapoint.*: datapointId, featureVector, sparseEmbedding,
restricts, numericRestricts, crowdingTag
#
# Numeric restricts accept ONE of valueInt/valueFloat/valueDouble with
'op' enum.
...
'sparseEmbedding' => dp['sparseEmbedding'].presence, # { values:
[Float], dimensions: [Integer] }
'restricts' => dp['restricts'].presence, # [{ namespace, allowList[],
denyList[] }]
'numericRestricts'=> dp['numericRestricts'].presence,# [{ namespace, op,
valueInt|valueFloat|valueDouble }]
...
q['perCrowdingAttributeNeighborCount'],
'fractionLeafNodesToSearchOverride' =>
q['fractionLeafNodesToSearchOverride'],
'rrf' => q['rrf'].presence # optional future-proofing; if you need RRF
fusion
}.compact
end
{
...
hash[element['name']] = [call('format_parse_sample',
element['properties'])]
elsif element['type'] == 'object'
hash[element['name']] = call('format_parse_sample',
element['properties'])
else
hash[element['name'].gsub(/^\d|\W/) { |c| "_ #{c.unpack('H*')}" }] =
'<Sample Text>'
end
end || {}
end,
# --- Dynamic Model Discovery for Vertex AI (Model Garden/Publisher
models) ---
fetch_publisher_models: lambda do |connection, publisher = 'google'|
# Use the regional service endpoint; list is in v1beta1.
# Docs: publishers.models.list (v1beta1), supports 'view' and
pagination.
#
...
view: 'PUBLISHER_MODEL_VIEW_FULL', # include launchStage/versionState,
etc.
list_all_versions: true
)
.after_error_response(/.*/) do |code, body, _hdrs, message|
error("List publisher models failed (HTTP #{code}) - #{message}:
#{body}")
end
...
return :embedding if id.include?('embedding') || id.include?('gecko') ||
id.include?('multimodalembedding') || id.include?('embeddinggemma')
...
# Prefer GA and stable versions unless explicitly asked to include
preview.
...
include_preview || stage == 'GA' || stage.blank? # keep if GA or unknown
when preview is off
end
# make ids unique by name
ids = filtered.map { |m| m['name'].to_s.split('/').last }.compact.uniq
options = ids.map do |id|
label = id.gsub('-', ' ').split.map { |t| t =~ /\d/ ? t : t.capitalize
}.join(' ')
...
[
(label[/\b2\.5\b/] ? 0 : label[/\b2\.0\b/] ? 1 : label[/\b1\.5\b/] ? 2 :
3),
(label[/\bPro\b/] ? 0 : label[/\bFlash\b/] ? 1 : label[/\bLite\b/] ? 2 :
3),
label
]
end
end,
# High-level API used by pick_lists. Falls back to your static lists
when listing is off or fails.
...
# Prefer v1 get (has launchStage); BASIC view is default; ask FULL to be
safe.
#
...
.after_error_response(/.*/) do |code, body, _hdrs, message|
error("Model validation failed (HTTP #{code}) for #{model_name} in
#{region}: #{message}: #{body}")
end
...
"Uncheck 'Validate model before run' or enable 'Include
...
error("To use 'Workato data table' as rules source, please configure
connection fields: #{missing.join(', ')}") if missing.present?
end,
...
(connection['workato_api_host'] ||
'https://www.workato.com').gsub(%r{/\z}, '')
end,
list_datatables: lambda do |connection|
call('ensure_workato_api!', connection)
base = call('workato_api_base', connection)
get("#{base}/api/data_tables")
.headers(call('workato_api_headers', connection))
.after_error_response(/.*/) { |code, body, _h, msg| error("Data tables
list failed (HTTP #{code}) - #{msg}: #{body}") }
end,
list_datatable_columns: lambda do |connection, table_id|
call('ensure_workato_api!', connection)
base = call('workato_api_base', connection)
get("#{base}/api/data_tables/#{table_id}")
.headers(call('workato_api_headers', connection))
.after_error_response(/.*/) { |code, body, _h, msg| error("Get data
table failed (HTTP #{code}) - #{msg}: #{body}") }
end,
...
.headers(headers)
.after_error_response(/.*/) { |code, body_s, _h, msg| error("Query rows
failed (HTTP #{code}) - #{msg}: #{body_s}") }
...
# Minimal validation; if connection creds aren’t present we error in
ensure_workato_api!
...
# connection not passed here; Workato provides it to execute/payload via
closure binding
...
hint: ' A token is approximately four characters. 100 tokens correspond
to ' \
...
sticky: true },
{ name: 'topK',
hint: 'Specify a lower value for less random responses and a higher
value for ' \
...
hint: 'Positive values penalize tokens that already appear in the
generated ' \
...
convert_input: 'float_conversion' },
{ name: 'logitBias',
hint: 'Mapping of token IDs to their bias values. The bias values are
added ' \
'to the logits before sampling. Allowed values: From -100 to 100.' },
{ name: 'stopSequences',
type: 'array', of: 'string',
hint: 'Specifies a list of strings that tells the model to stop
generating text ' \
...
{ name: 'toolConfig',
type: 'object',
hint: 'This tool config is shared for all tools provided in the
request.',
group: 'Tools',
properties: [
...
control_type: 'number',
convert_input: 'float_conversion',
hint: 'A number that controls the probability of the model generating
each token. ' \
'A higher topP will result in the model generating more likely tokens,
while a ' \
...
control_type: 'number',
convert_input: 'float_conversion',
hint: 'A number that controls the number of tokens that the model
considers when ' \
'generating each token. A higher topK will result in the model
considering more ' \
'tokens, while a lower topK will result in the model considering fewer
tokens. ' \
...
properties: [
{ name: 'role',
hint: 'The role should be <b>model</b> when defining system
instructions.' },
{ name: 'parts',
type: 'array',
of: 'object',
...
descriptions ' \
'for extracting the fields. Required fields take effect only on top
level. ' \
...
next [] if config_fields['object_schema'].blank?
        # Parse the provided object schema for the output fields.  Use
        # the safe parser to surface a clear error if the JSON is
        # malformed.
        schema = call('safe_parse_json', config_fields['object_schema'] || '[]', 'object schema')
...
{ name: 'rule', label: 'Rule (optional)', hint: 'E.g. subject contains:
“invoice”' }
],
group: 'Rules'
},
...
pick_list: :workato_datatable_columns,
pick_list_params: { table_id: 'rules_table_id' },
hint: 'Optional column with rule text (e.g., patterns). If blank, only
category names will be used.',
toggle_hint: 'Select from list',
toggle_field: {
name: 'rule_column',
type: 'string',
...
control_type: 'checkbox',
sticky: true,
hint: 'If your table has an "active" boolean column, only include rows
where active = true',
group: 'Filter'
},
{
...
control_type: 'text-area',
optional: false,
hint: 'Input text must not exceed 8192 tokens (approximately 6000
words).',
group: 'Text' },
{ name: 'task_type',
sticky: true,
...
hint: 'Provide the intended downstream application to help the model
produce ' \
...
optional: false,
hint: 'The host for the index endpoint (no path). Example:
1234.us-central1.vdb.vertexai.goog '\
'for public endpoints, or your PSC DNS/IP. Do NOT include https://.',
group: 'Endpoint' },
{ name: 'index_endpoint_id',
label: 'Index endpoint ID',
optional: false,
hint: 'Resource ID only (not full path). Find it on the Index endpoint
details page.',
group: 'Index context' },
...
type: 'boolean',
control_type: 'checkbox',
hint: 'If enabled, returns full vectors and restricts. Increases latency
and cost.',
group: 'Options' },
{
name: 'queries',
...
{ name: 'featureVector', label: 'Feature vector', type: 'array', of:
'number',
hint: 'Dense embedding (float array). Provide either a vector or an ID
(or both).' },
...
toggle_hint: 'Select from list',
toggle_field: { name: 'op', type: 'string', control_type: 'text',
optional: true,
toggle_hint: 'Use custom value' } },
...
], hint: 'Optional; used to improve result diversity by limiting
same-tag neighbors.' }
]
},
{ name: 'neighborCount', type: 'integer', control_type: 'integer',
hint: 'k — number of neighbors to return.' },
{ name: 'approximateNeighborCount', type: 'integer', control_type:
'integer',
...
%w[EQUAL NOT_EQUAL LESS LESS_EQUAL GREATER GREATER_EQUAL].map { |m|
[m.humanize, m] }
end,
...
[
'Albanian', 'Arabic', 'Armenian', 'Awadhi', 'Azerbaijani', 'Bashkir',
'Basque',
'Belarusian', 'Bengali', 'Bhojpuri', 'Bosnian', 'Brazilian Portuguese',
'Bulgarian',
'Cantonese (Yue)', 'Catalan', 'Chhattisgarhi', 'Chinese', 'Croatian',
'Czech', 'Danish',
'Dogri', 'Dutch', 'English', 'Estonian', 'Faroese', 'Finnish', 'French',
'Galician',
'Georgian', 'German', 'Greek', 'Gujarati', 'Haryanvi', 'Hindi',
'Hungarian', 'Indonesian', 'Irish', 'Italian', 'Japanese', 'Javanese',
'Kannada',
'Kashmiri', 'Kazakh', 'Konkani', 'Korean', 'Kyrgyz', 'Latvian',
'Lithuanian',
'Macedonian', 'Maithili', 'Malay', 'Maltese', 'Mandarin', 'Mandarin
Chinese', 'Marathi',
'Marwari', 'Min Nan', 'Moldovan', 'Mongolian', 'Montenegrin', 'Nepali',
'Norwegian',
'Oriya', 'Pashto', 'Persian (Farsi)', 'Polish', 'Portuguese', 'Punjabi',
'Rajasthani',
'Romanian', 'Russian', 'Sanskrit', 'Santali', 'Serbian', 'Sindhi',
'Sinhala', 'Slovak',
'Slovene', 'Slovenian', 'Swedish', 'Ukrainian', 'Urdu', 'Uzbek',
'Vietnamese',
'Welsh', 'Wu'
]
end,
